  ...
  <body>
    ...
    <main class="main">
      <div class="main__contenido">
        ...
        <h1 class="agregar__titulo"> TIPO COMPROBANTE</h1>
          <div class="listar__buscador">
            ...
            <div class="contenedor-boton-crear">
              <a class="boton-crear boton" href="vista-agregar.php?token=<?php echo encriptar_parametro("1|5"); ?>"><i class="fas fa-plus-circle"></i>Crear nuevo</a>
            </div>
          </div>
          <div class="listar__tabla">
            <table>
              ...
              <?php
                    ...
                    {
                          //ID
                          $p_rptJson = $objDB->tipocom_listar_porid($p_campo_dato);
                    }else
                    if (($p_campo_buscar =='02') && ($p_campo_dato <> ''))
                    {
                          //Descripcion
                          $p_rptJson = $objDB->tipocom_listar_pordescripcion($p_campo_dato);
                    }else  
                    {                 
                          //all
                          $p_rptJson = $objDB->tipocom_listar();
                    }

                    $p_itemsTipocom = json_decode($p_rptJson, true);

                    foreach ($p_itemsTipocom as $p_itemTipocom) 
                    {
                        $pcodigo = $p_itemTipocom['tipocomprobante_id'];
              ?>

                        <tr>
                          <td class="listar__tabla--id"><?php echo $p_itemTipocom['tipocomprobante_id']; ?></td>
                          <td class="listar__tabla--descripcion"><?php echo $p_itemTipocom['descripcion']; ?></td>
                          <td class="listar__tabla--estado"><?php echo $p_itemTipocom['estado']; ?></td>
                          <td class="listar__tabla--acciones">    
                          <div class="boton-aciones"><a href="vista-editar.php?token=<?php echo encriptar_parametro("1|5|".$pcodigo); ?>" class="boton-aciones__editar">Editar</a>
                            <a href="vista-eliminar.php?token=<?php echo encriptar_parametro("1|5|".$pcodigo); ?>" class="boton-aciones__eliminar">Eliminar</a>
                          </div>
                        </td> 
